#!/usr/bin/env tsx
/**
 * migrate-2024-blog.ts
 *
 * Migrates blog posts from Blog/src/2024/** to Website-v2/src/content/blog/2024/**
 ...
 * - Replaces local image refs with plain Cloudinary URLs (no transform — added at build time)
 * - First image is copied to the cover frontmatter field and kept in the body
 *
 * Usage:
 *   npx tsx scripts/migrate-2024-blog.ts [--dry-run]
 */
...
const CLOUDINARY_FOLDER = "bygregmarine/blog/2024";
const REPO_ROOT = path.resolve(import.meta.dirname, "../..");
const SOURCE_ROOT = path.join(REPO_ROOT, "Blog/src/2024");
const DEST_ROOT = path.join(REPO_ROOT, "Website-v2/src/content/blog/2024");
...
interface ImageRef {
  ...
  fullMatch: string;
  /** Relative path from the post dir, e.g. img/foo.jpg or media/foo.jpg */
  localPath: string;
...
function extractTitle(
  ...
  if (h1Index !== -1) {
    ...
    // Remove leading blank lines from the remaining body
...
function slugToFilename(basename: string): string {
  // Strip DD_ prefix and normalize remaining underscores to hyphens
  // e.g. 05_new-year-new-blog.md -> new-year-new-blog
  // e.g. 26_disney_christmas.md  -> disney-christmas
...
async function processPost(srcPath: string): Promise<void> {
  ...
  const dateStr = `2024-${mm}-${dd}T00:00:00Z`;
